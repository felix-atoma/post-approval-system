  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.data?.passwordReset || error.response?.data?.code === 'PASSWORD_RESET_REQUIRED') {
        // This is handled in the login function - redirects to create-password
        return;
      }
      
      // Other errors are already handled in auth context
        ...
        <div className="bg-white py-8 px-4 shadow-xl rounded-lg sm:px-10 border border-gray-200">
          ...
          <div className="mb-6 p-4 bg-blue-50 rounded-md border border-blue-100">
            <p className="text-sm text-blue-800">
              <strong className="font-semibold">New users created by admin:</strong> Enter your email and any temporary password. 
              You'll be prompted to set your permanent password on the next screen.
            </p>
          </div>
          ...
          <div className="mt-8">
            ...
            <div className="mt-6 space-y-3">
              ...
              <div 
                className="bg-yellow-50 border border-yellow-100 rounded-lg p-4 transition-all hover:bg-yellow-100 cursor-pointer"
                onClick={() => !isSubmitting && handleDemoLogin('newuser@example.com', 'temp123')}
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-yellow-800">New User (Password Reset)</p>
                    <p className="text-xs text-yellow-600 mt-1">Created by admin - needs password setup</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-mono text-yellow-700">newuser@example.com</p>
                    <p className="text-xs text-yellow-600">Any temporary password</p>
                  </div>
                </div>
              </div>
            </div>
